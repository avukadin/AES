...
void s_box_encrypt(char *a);
vector <char> round_const(int round);

int main(){
    int end=1;
    while (end==1){
    cout<<endl<<"This software uses 128-bit AES to encrypt files"<<endl<<endl;
    cout<<"Choose 0-encrypt, 1-decrypt:";
    int encrypt;
    cin>>encrypt;
    if(encrypt==0){
        cout<<endl<<"Enter file name to be encrypted:";
    }

    else{
        cout<<endl<<"Enter file name to be decrypted:";
    }
    string file;
    cin>>file;
    ...
    while(true){
        ...
        if(key.size()<=16 && key.size()>=4){
            break;
        }

        else{
            cout<<"Error: key must be between 4-16 characters"<<endl;

        }
    }
    int extra=0;
    source_matrix master(file,&extra);
   ...
   master.save_file(file, extra, encrypt);
   cout<<"Choose: 0-exit, 1-restart";
   cin>>end;
   }
   return 0;
}
